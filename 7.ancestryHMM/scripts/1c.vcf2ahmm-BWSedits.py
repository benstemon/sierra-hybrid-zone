    ...
    for line in csv.reader(tsv, delimiter="\t") :
        ...
        for s in range(9, len( line ) ) :
            if ( sample_files[s] in sample2pop ) :
                ...
                if ( line[s].startswith('0/0') or line[s].startswith('0|0')) :
                    totals[sample2pop[sample_files[s]]] += 2
                elif ( line[s].startswith('0/1') or line[s].startswith('0|1') or line[s].startswith('1|0')) :
                    alts[sample2pop[sample_files[s]]] += 1
                    totals[sample2pop[sample_files[s]]] += 2
                elif ( line[s].startswith('1/1') or line[s].startswith('1|1')) :
